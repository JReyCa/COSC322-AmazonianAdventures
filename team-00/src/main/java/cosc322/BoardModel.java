...
public class BoardModel extends GameModel {
    ...
    public String validateMove(int[] oldQueenPosition, int[] newQueenPosition, int[] arrowPosition) {
        ...
        if (isOutOfBounds(oldQueenPosition, this)) {
            ...
            //dont try and move or shoot an arrow on an occupied tile
        } else if (isOccupied(newQueenPosition, arrowPosition, this)){
            message = TARGET_OCCUPIED;
        }
    ...
    //check if the landing space is occupied 
    private static boolean isOccupied(int[] newQueenPos, int[] arrowPos, BoardModel model) {

        String queenSpace = model.getTile(newQueenPos);
        String arrowSpace = model.getTile(arrowPos);

        if (!queenSpace.equals(POS_AVAILABLE) || !arrowSpace.equals(POS_AVAILABLE)) {
            return true;
        }
